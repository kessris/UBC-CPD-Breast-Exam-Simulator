...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] DoctorNetworkControllerNetworkObject = null;
		public GameObject[] DoorNetworkObject = null;
		public GameObject[] ExampleProximityPlayerNetworkObject = null;
		public GameObject[] NetworkCameraNetworkObject = null;
		public GameObject[] PauseScreenNetworkObject = null;
		public GameObject[] PlayerNetworkObject = null;
		public GameObject[] SimplePictureNetworkObject = null;
		public GameObject[] TestNetworkObject = null;
		public GameObject[] ObserverNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is DoctorNetworkControllerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (DoctorNetworkControllerNetworkObject.Length > 0 && DoctorNetworkControllerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(DoctorNetworkControllerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<DoctorNetworkControllerBehavior>();
						}
					}
			...
			else if (obj is DoorNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (DoorNetworkObject.Length > 0 && DoorNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(DoorNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<DoorBehavior>();
						}
					}
			...
			else if (obj is PauseScreenNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (PauseScreenNetworkObject.Length > 0 && PauseScreenNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(PauseScreenNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<PauseScreenBehavior>();
						}
					}
			...
			else if (obj is SimplePictureNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (SimplePictureNetworkObject.Length > 0 && SimplePictureNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(SimplePictureNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<SimplePictureBehavior>();
						}
					}
			...
			else if (obj is ObserverNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (ObserverNetworkObject.Length > 0 && ObserverNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(ObserverNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<ObserverBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateDoctorNetworkController instead, its shorter and easier to type out ;)")]
		public DoctorNetworkControllerBehavior InstantiateDoctorNetworkControllerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(DoctorNetworkControllerNetworkObject[index]);
			var netBehavior = go.GetComponent<DoctorNetworkControllerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<DoctorNetworkControllerBehavior>().networkObject = (DoctorNetworkControllerNetworkObject)obj;

			FinalizeInitialization(go, netBehavior, obj, position, rotation, sendTransform);
			
			return netBehavior;
		}
		[Obsolete("Use InstantiateDoor instead, its shorter and easier to type out ;)")]
		public DoorBehavior InstantiateDoorNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(DoorNetworkObject[index]);
			var netBehavior = go.GetComponent<DoorBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<DoorBehavior>().networkObject = (DoorNetworkObject)obj;
		...
		[Obsolete("Use InstantiatePauseScreen instead, its shorter and easier to type out ;)")]
		public PauseScreenBehavior InstantiatePauseScreenNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(PauseScreenNetworkObject[index]);
			var netBehavior = go.GetComponent<PauseScreenBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<PauseScreenBehavior>().networkObject = (PauseScreenNetworkObject)obj;
		...
		[Obsolete("Use InstantiateSimplePicture instead, its shorter and easier to type out ;)")]
		public SimplePictureBehavior InstantiateSimplePictureNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(SimplePictureNetworkObject[index]);
			var netBehavior = go.GetComponent<SimplePictureBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<SimplePictureBehavior>().networkObject = (SimplePictureNetworkObject)obj;
		...
		[Obsolete("Use InstantiateObserver instead, its shorter and easier to type out ;)")]
		public ObserverBehavior InstantiateObserverNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(ObserverNetworkObject[index]);
			var netBehavior = go.GetComponent<ObserverBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<ObserverBehavior>().networkObject = (ObserverNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of DoctorNetworkController
		/// </summary>
		/// <returns>
		/// A local instance of DoctorNetworkControllerBehavior
		/// </returns>
		/// <param name="index">The index of the DoctorNetworkController prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public DoctorNetworkControllerBehavior InstantiateDoctorNetworkController(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			if (DoctorNetworkControllerNetworkObject.Length <= index)
			{
				Debug.Log("Prefab(s) missing for: DoctorNetworkController. Add them at the NetworkManager prefab.");
				return null;
			}
		...
		/// <summary>
		/// Instantiate an instance of Door
		/// </summary>
		/// <returns>
		/// A local instance of DoorBehavior
		/// </returns>
		/// <param name="index">The index of the Door prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public DoorBehavior InstantiateDoor(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			if (DoorNetworkObject.Length <= index)
			{
				Debug.Log("Prefab(s) missing for: Door. Add them at the NetworkManager prefab.");
				return null;
			}
		...
		/// <summary>
		/// Instantiate an instance of PauseScreen
		/// </summary>
		/// <returns>
		/// A local instance of PauseScreenBehavior
		/// </returns>
		/// <param name="index">The index of the PauseScreen prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public PauseScreenBehavior InstantiatePauseScreen(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			if (PauseScreenNetworkObject.Length <= index)
			{
				Debug.Log("Prefab(s) missing for: PauseScreen. Add them at the NetworkManager prefab.");
				return null;
			}
		...
		/// <summary>
		/// Instantiate an instance of SimplePicture
		/// </summary>
		/// <returns>
		/// A local instance of SimplePictureBehavior
		/// </returns>
		/// <param name="index">The index of the SimplePicture prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public SimplePictureBehavior InstantiateSimplePicture(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			if (SimplePictureNetworkObject.Length <= index)
			{
				Debug.Log("Prefab(s) missing for: SimplePicture. Add them at the NetworkManager prefab.");
				return null;
			}
		...
		/// <summary>
		/// Instantiate an instance of Observer
		/// </summary>
		/// <returns>
		/// A local instance of ObserverBehavior
		/// </returns>
		/// <param name="index">The index of the Observer prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public ObserverBehavior InstantiateObserver(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			if (ObserverNetworkObject.Length <= index)
			{
				Debug.Log("Prefab(s) missing for: Observer. Add them at the NetworkManager prefab.");
				return null;
			}
